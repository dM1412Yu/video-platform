...
let answered = false;
// 新增：记录已触发弹窗的知识点索引，防止重复弹窗
let triggeredSegIndex = -1;

// ===================== 新增：按视频时间自动触发弹窗核心逻辑 =====================
video.addEventListener('timeupdate', function() {
  const currentVideoTime = video.currentTime;
  // 遍历所有知识点，判断是否播放到知识点起始时间
  segments.forEach((seg, index) => {
    const segStartTime = parseFloat(seg.start_time) || 0;
    // 播放到知识点起始时间±0.5秒内，且该知识点未触发过弹窗
    if (currentVideoTime >= segStartTime - 0.5 && currentVideoTime <= segStartTime + 0.5 && triggeredSegIndex !== index) {
      triggeredSegIndex = index; // 标记为已触发，防止重复
      openQuizPopupByTime(index); // 触发弹窗
    }
  });
});

// 新增：根据时间触发弹窗的具体逻辑
function openQuizPopupByTime(segIndex) {
  // 从knowledge_questions.json中获取对应知识点的题目
  const questionItem = questionsData.find(item => item.title === segments[segIndex].title);
  if (!questionItem || !questionItem.questions || questionItem.questions.length === 0) {
    return; // 无题目则不弹窗
  }
  
  // 初始化弹窗数据
  currentSeg = questionItem;
  currentQuestionIdx = 0; // 默认显示引导题
  answered = false;
  
  // 设置弹窗标题和重置状态
  quizTitle.innerText = `引导题：${questionItem.questions[0].question}`;
  userAnswer.value = '';
  feedback.innerText = '';
